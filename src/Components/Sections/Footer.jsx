        ...
        <div className="w-full bg-zafiro text-white flex justify-center items-center pt-6 pb-12">
            <div className="w-10/12 xl:w-8/12 flex flex-col md:flex-row justify-between gap-4 xl:gap-6">
                <div className="md:w-[15%] flex flex-col md:mt-8">
                    ...
                    { list1.map( (item, index) => ( <a className={ listItemCss } key={ `key-${item}${index}` }>{ item }</a> )) }
                </div>
                <div className="md:w-[15%] flex flex-col md:mt-8">
                    <p className={ titleCss }>{ title2 }</p>
                    { list2.map( (item, index) => ( <a className={ listItemCss } key={ `key-${item}${index}` }>{ item }</a> )) }
                </div>
                ...
                <div className="md:w-[15%] flex flex-col md:mt-8">
                    ...
                    { list3.map( (item, index) => ( <a className={ listItemCss } key={ `key-${item}${index}` }>{ item }</a> )) }
                </div>
                <div className="md:w-[15%] flex flex-col md:mt-8">
                    <p className={ titleCss }>{ title4 }</p>
                    { list4.map( (item, index) => ( <a className={ listItemCss } key={ `key-${item}${index}` }>{ item }</a> )) }
                </div>
            </div>
        </div>
    )
